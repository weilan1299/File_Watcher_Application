...
class Tkinter_GUI(View):
    ...
    def create_widgets(self):
        ...
        start_watch_btn = CTkButton(self.frame1, text="Start Watching", corner_radius=20,
                                         cursor="hand2", border_color="#FFCC70", border_width=3,
                                         command=self.start_monitoring)

        start_watch_btn.grid(row=0, column=0, padx=5, pady=5)

        stop_watch_btn = CTkButton(self.frame1, text="Stop Watching", corner_radius=20,
                                        cursor="hand2", border_color="#FFCC70", border_width=3,
                                        command=self.stop_monitoring)
        stop_watch_btn.grid(row=0, column=1, padx=5, pady=5)

        reset_btn = CTkButton(self.frame1, text="Reset", corner_radius=20,
    ...
    def q_email_data(self):
        ...
        # To make the root window disabled while the new_window is still open
        new_window.transient(self.__root)
        new_window.grab_set()
